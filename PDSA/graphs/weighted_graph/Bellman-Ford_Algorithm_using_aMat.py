...
class Weighted_graph:
    ...
    def bellman_ford(self, source):
        """
        This method implements the Bellman-Ford algorithm.
        """
        (rows, cols, x) = np.shape(self.aMat)
        distance = {t:float('inf') for t in range(cols)}
        ...
        for i in range(rows):
            for u in range(rows):
                for v in range(rows):
                    if self.aMat[u,v,0] == 1 :
                        distance[v] = min(distance[v], distance[u]+self.aMat[u,v,1])
        return distance
